#This code is for laoding the data into user_visit_details,user_profile_details table from userdetails.json file.

#userdetails file data is modeled into 2 different data set , And user visit transaction details are stored in user_visit_details table by the visit_date partition.

#Another table user_profile_details is dimension contains the user attributes and loading in overwrite based on the latest data .

# Added indicator 1 to data from the file and indicator 2 for the data in table. Used row_number function to determine which one is latest based on indicator. Combined both data frames from file and dimensional table and filtered where row_number = 1 to load updated place details in user_profile_details table.
...
def updateUserDetails(spark,inputpath):
    ...
    if userDetails_df.limit(1).count() > 0:
        ...
        userVisitDetails_df.withColumn("visitdt",substrint(col("visitdate"),1,10)).write.mode(SaveMode.Append).partitionBy("visitdt").insertInto("rubric.user_visit_details") \
        ...
        fullUserDetails_df=spark.table("rubric.user_profile_details").withColumn("ind",lit(2))
        ...
        latestUserProfileDetails_df.write.mode(SaveMode.Overwrite).insertInto("rubric.user_profile_details")
